...
class Character(pygame.sprite.Sprite):
    def __init__(self, pos, group):
        ...
        # Boolean to check if character is talking with an NPC or not
    ...
    def import_assets(self):
        ...
        self.animations = {
            ...
            'up_stab': [], 'down_stab': [], 'right_stab': [], 'left_stab': [],
        }
        ...
        sprite_sheets = {
            ...
            './assets/textures/character/character_stab.png': {
                'rows': 4, 'cols': 4, 'animations': ['down_stab', 'up_stab', 'right_stab', 'left_stab']
            }
        }
    ...
    def attack(self):
        # Check if attack_bool is True
        if self.attack_bool:
            # Calculate the offset based on the direction the character is facing
            offset = pygame.math.Vector2(0, 0)

            if self.status == 'up' or self.status == 'up_idle' or self.status == 'up_stab':
                offset.y = -25
            elif self.status == 'down' or self.status == 'down_idle' or self.status == 'down_stab':
                offset.y = 25
            elif self.status == 'left' or self.status == 'left_idle' or self.status == 'left_stab':
                offset.x = -25
            elif self.status == 'right' or self.status == 'right_idle' or self.status == 'right_stab':
                offset.x = 25
    ...
    def input(self):
        ...
        if not self.death_bool:
            ...
            if self.sprinting:
                ...
                if self.stamina == 0:
                    # Set speed to walking speed if stamina is 0 (out of stamina)
    ...
    def get_status(self):
        ...
        if self.attack_bool:
            self.status = self.status.split('_')[0] + '_stab'
    ...
    def draw_stamina_bar(self, display_surface, dt):
        if self.sprinting_bool:  # If sprinting, degenerate stamina
            ...
            # Check if not moving and stamina is less than max stamina
                ...
                # If stamina is greater than or equal to max stamina, set stamina to max stamina
        ...
        self.stamina_bar_width = OVERLAY_POSITIONS['Stamina']['size'][0]
        # Height of the stamina bar
        self.stamina_bar_height = OVERLAY_POSITIONS['Stamina']['size'][1]
        # X-coordinate of the top-left corner of the stamina bar
        self.stamina_bar_x = OVERLAY_POSITIONS['Stamina']['position'][0]
        # Y-coordinate of the top-left corner of the stamina bar
        self.stamina_bar_y = OVERLAY_POSITIONS['Stamina']['position'][1]

        # Calculate the current stamina bar width based on the current stamina value
        self.current_stamina_width = (
            self.stamina / self.max_stamina) * self.stamina_bar_width
        # Create the stamina text surface
        text_surface = self.font.render('Stamina:', True, (0, 0, 0))
        # Draws text above the stamina bar
        display_surface.blit(
            text_surface, (self.stamina_bar_x, self.stamina_bar_y))
    ...
    def draw_health_bar(self, display_surface, dt):
        if self.health_bool:  # If health_bool is true, regenerate health
            ...
            if self.health <= 0:  # If health is less than or equal to 0, set health to 0
                ...
                # Create a thread to kill the character after 5 seconds
